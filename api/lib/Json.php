...
class Json {
    ...
    {
        $dados = [];
        $dados[ConstantesGenericas::TIPO] = ConstantesGenericas::TIPO_ERRO;

        if ((is_array($retorno) && count($retorno) > 0) || strlen($retorno) > 10) {
            $dados[ConstantesGenericas::TIPO] = ConstantesGenericas::TIPO_SUCESSO;
            $dados[ConstantesGenericas::RESPOSTA] = $retorno;
        }
